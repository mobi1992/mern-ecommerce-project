...
import CartContainer from '../../@components/unknownUserCart'
const CartUnknownUser = ({ prod }) => {
    const [cartCon, showCartCon] = useState(false)
    const [quantity, setQuantity] = useState(1)
    const dispatch = useDispatch()
    const { addToCartUnknownUser, error, loading } = useSelector(state => state.addToCartUnknownUser)
    ...
    if (error) {
        ...
        return (
            <>
                {loading ? <div></div> : <Row style={{ marginLeft: '0.5vw' }} className='main'>
                    <Col>
        ...
        <>
            {loading ? <div></div> : <Row style={{ marginLeft: '0.5vw' }} className='main'>
                ...
                {cartCon && <CartContainer onCloseCart={onCloseCart} msg={addToCartUnknownUser.message} />}
            </Row>}
        </>
    )
        }
